    ...
    for video_id in sorted(set(video_ids)):
        ...
        for file_name in file_names:
            ...
            if file_name.endswith(".zip") and os.path.isfile(dst_path):
                ...
                # Delete unwanted folders to save disk space.

                # try:
                video_dirpath = Path(dst_dir) / Path(file_name).stem
                shutil.rmtree(video_dirpath / "confidence")
                shutil.rmtree(video_dirpath / "lowres_depth")
                shutil.rmtree(video_dirpath / "highres_depth")
                # # Move all files into same directory with the appropriate prefixes.
                # images_path = video_dirpath / "wide"
                # highres_depth_path = video_dirpath / "highres_depth"
                # for f in images_path.iterdir():
                #     shutil.move(f, video_dirpath / f"rgb_{f.name}")
                # for f in highres_depth_path.iterdir():
                #     shutil.move(f, video_dirpath / f"depth_{f.name}")
                # shutil.rmtree(images_path)
                # shutil.rmtree(highres_depth_path)
                # except:
...
if __name__ == "__main__":
    ...
    assert (
        args.video_id is not None or args.video_id_csv is not None
    ), "video_id or video_id_csv must be specified"
    assert (
        args.video_id is None or args.video_id_csv is None
    ), "only video_id or video_id_csv must be specified"
    assert (
        args.video_id is None or args.split is not None
